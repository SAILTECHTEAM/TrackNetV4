# vball_net_v3c_optimized.py
# Optimized version of VballNetV3c with improved accuracy while maintaining speed
# Key improvements:
# 1. Lightweight attention mechanisms
# 2. Efficient temporal aggregation
# 3. Optimized motion fusion
...
def power_normalization(x: torch.Tensor, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    """Optimized power normalization with better gradient flow.
...
class LightweightChannelAttention(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=8):
        ...
        mid_channels = max(in_channels // reduction_ratio, 4)
...
class MotionPromptLayer(nn.Module):
    """Optimized motion prompt with efficient temporal difference calculation.
...
class FusionLayerTypeA(nn.Module):
    """Efficient attention projection (T -> out_dim) via 1x1 conv.
...
class VballNetV3cOptimized(nn.Module):
    """Optimized VBallNet v3c - improved accuracy while maintaining speed.
    ...
    def __init__(self, height: int = 288, width: int = 512, in_dim: int = 9, out_dim: int = 9):
        ...
        self.temporal_ca = LightweightChannelAttention(10)
        ...
        # Lightweight output enhancement
        self.output_ca = LightweightChannelAttention(out_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # === Optimized Temporal 3D extractor ===
        ...
        # Apply lightweight attention to output
...
if __name__ == "__main__":
    ...
    model = VballNetV3cOptimized(height=288, width=512, in_dim=9, out_dim=9).to(device)
    model.train()
    ...
    try:
        torch.onnx.export(
            model,
            (x,),
            "vball_net_v3c_optimized.onnx",
            opset_version=17,
            input_names=["clip"],
            output_names=["heatmaps"],
            dynamic_axes={"clip": {0: "B"}, "heatmaps": {0: "B"}},
        )
        print("ONNX saved: vball_net_v3c_optimized.onnx")
