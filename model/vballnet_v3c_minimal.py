# vball_net_v3c_minimal.py
# Minimal improvements to VballNetV3c for better accuracy with negligible speed impact
# Key improvements:
# 1. Better numerical stability in power normalization
# 2. Improved temporal difference calculation
...
def power_normalization(x: torch.Tensor, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    """Numerically stable power normalization.
    
    x: (...)
    a, b: scalar parameters
    """
    abs_x = torch.abs(x)
    denom = 0.45 * torch.abs(torch.tanh(a)) + 1e-3  # Better epsilon for stability
    scale = 5.0 / denom
    return torch.sigmoid(scale * (abs_x - 0.8 * torch.tanh(b)))


class MotionPromptLayer(nn.Module):
    """Motion prompt with improved temporal difference calculation.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Improved smoothness penalty
        if self.training and self.penalty_weight > 0:
            diff_temporal = attention[:, 1:] - attention[:, :-1]
            # Use L1 norm for sparser gradients
            smoothness = torch.mean(torch.abs(diff_temporal))
...
class FusionLayerTypeA(nn.Module):
    """Attention projection (T -> out_dim) via 1x1 conv.
...
class VballNetV3cMinimal(nn.Module):
    """Minimal improvements to VBallNet v3c for better accuracy with negligible speed impact.
    ...
    def __init__(self, height: int = 288, width: int = 512, in_dim: int = 9, out_dim: int = 9):
        ...
        # --- Temporal Conv3D ---
        # Same as original but with better initialization
        ...
        # Motion guidance with improvements
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Temporal aggregation - same as original but clearer
...
if __name__ == "__main__":
    ...
    model = VballNetV3cMinimal(height=288, width=512, in_dim=9, out_dim=9).to(device)
    model.train()
    ...
    try:
        torch.onnx.export(
            model,
            (x,),
            "vball_net_v3c_minimal.onnx",
            opset_version=17,
            input_names=["clip"],
            output_names=["heatmaps"],
            dynamic_axes={"clip": {0: "B"}, "heatmaps": {0: "B"}},
        )
        print("ONNX saved: vball_net_v3c_minimal.onnx")
